...
class MessageParser
{
    ...
    {
        $first_space = strpos($message, ' ');
        $this->ircUser = trim(substr($message, 0, $first_space));
    ...
    {
        $first_space = strpos($message, ' ');
        $this->ircCommand = trim(substr($message, 0, $first_space));
    ...
    private function parseIrcRoom(string $message): string
    {
        ...
        if ($first_space === false) {
            ...
            $this->ircRoom = trim($message);
            return '';
        } else {
            $this->ircRoom = trim(substr($message, 0, $first_space));
